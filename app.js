...
async function loadGithubActivity() {
  ...
  try {
    ...
    followerCountEl.textContent = user.followers ?? '--';

    const eventsRes = await fetch(`https://api.github.com/users/${GITHUB_USERNAME}/events/public`);
    if (eventsRes.ok) {
      const events = await eventsRes.json();
      if (events.length > 0) {
        const daysAgo = Math.floor((Date.now() - new Date(events[0].created_at)) / 86400000);
        lastActiveEl.textContent = daysAgo === 0 ? 'Today' : `${daysAgo}d ago`;
      } else {
        lastActiveEl.textContent = 'No recent activity';
      }
    }
